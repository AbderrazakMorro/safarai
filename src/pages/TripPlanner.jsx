...
async function fetchWikivoyageInfo(cityName) {
   ...
   try {
      ...
      sections.overview =
         firstHeading > 0
            ? fullText.substring(0, firstHeading).trim()
            : fullText.substring(0, 800).trim();
      while ((match = sectionRegex.exec(fullText)) !== null) {
         if (lastIdx > 0)
            sections[lastSection] = fullText.substring(lastIdx, match.index).trim();
      ...
      for (const key of interestingKeys) {
         for (const [sk, sv] of Object.entries(sections)) {
            if (sk.includes(key) && sv.length > 20) {
               result[key] =
                  sv.length > 600 ? sv.substring(0, 600).replace(/\s\S*$/, "…") : sv;
               break;
            }
         }
      }
...
function MapController({ points, activePoint, mode }) {
   ...
   useEffect(() => {
      ...
      if (activePoint) {
         ...
         map.setView(
            [lat, lng],
            mode === "explore" ? 15 : Math.max(map.getZoom(), 8),
...
export default function TripPlanner() {
   ...
   const { isSaved, savePlace, removePlace, saveTrip, savedTrips } =
      useSavedPlaces();
   ...
   const handleRoadtrip = async (e) => {
      ...
      try {
         ...
         if (data?.length > 0) {
            ...
            setError(
               "Could not generate a route. Please check city names and try again.",
            );
         }
      } catch {
   ...
   const handleExplore = async (e) => {
      ...
      try {
         ...
         if (data?.length > 0) {
            ...
            setError(
               "Could not find places. Please check the city name and try again.",
            );
         }
      } catch {
         setError("Network error. Please check your connection and try again.");
   ...
   const polylinePositions = itinerary
      ? itinerary.map((s) => [s.lat, s.lng])
      : [];
   const filteredPois = pois
      ? categoryFilter === "all"
         ? pois
         : pois.filter((p) => p.category?.toLowerCase() === categoryFilter)
      : [];
   ...
   const availableSections = wikiInfo
      ? Object.keys(wikiInfo).filter((k) => wikiInfo[k]?.length > 10)
      : [];
   const uniqueCategories = pois
      ? [...new Set(pois.map((p) => p.category?.toLowerCase()).filter(Boolean))]
      : [];
   ...
   return (
      <div className="trip-planner-container page-content glass min-h-screen pb-32 md:pb-8">
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-6 animate-fade-in-up pt-4 md:pt-0">
            <div className="max-w-xl">
               ...
               <p className="text-on-surface-variant text-base md:text-lg font-medium leading-relaxed">
                  Let our AI forge the perfect journey. Uncover hidden gems within any
                  Moroccan city.
               </p>
            </div>
         </div>
         ...
         <div
            className="flex w-full md:w-auto bg-surface-container-lowest border border-outline-variant/30 p-1.5 rounded-2xl mb-6 animate-fade-in-up shadow-sm"
         ...
         >
            ...
            >
               ...
               {mode !== "roadtrip" && itinerary && (
                  <span
                     className="absolute top-2 right-2 md:-top-1 md:-right-1 w-2.5 h-2.5 bg-emerald-400 rounded-full border border-white shadow-sm animate-pulse"
            ...
            >
               ...
               {mode !== "explore" && pois && (
                  <span
                     className="absolute top-2 right-2 md:-top-1 md:-right-1 w-2.5 h-2.5 bg-amber-400 rounded-full border border-white shadow-sm animate-pulse"
                     title={`${pois.length} places in ${exploreCity}`}
                  ></span>
               )}
            </button>
         </div>
         ...
         {mode === "roadtrip" && (
            <Card className="mb-4 p-6 bg-surface-container-lowest shadow-[0_12px_32px_rgba(25,28,28,0.06)] border border-primary/10 animate-slide-in">
               <form
                  onSubmit={handleRoadtrip}
                  className="flex flex-col md:flex-row gap-4 items-stretch md:items-end"
               >
                  <div className="flex-1 w-full">
                     <label className="text-sm font-bold text-stone-500 uppercase tracking-wider ml-1 mb-2 block">
                        Origin
                     </label>
                  ...
                  <div className="flex-1 w-full relative">
                     ...
                     <div className="relative">
                        ...
                        <input
                           ...
                           className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-surface-container-low border border-primary/30 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-on-surface font-semibold text-base"
                           required
                        />
                     </div>
                  </div>
                  <div className="w-full md:w-32">
                     ...
                     <input
                        ...
                        className="w-full px-4 py-3.5 rounded-xl bg-surface-container-low border border-outline-variant/30 focus:ring-2 focus:ring-primary/20 outline-none transition-all text-on-surface text-center font-semibold text-base"
                        required
                     />
                  </div>
                  <button
                  ...
                  >
                     {isGenerating ? (
                        <>
                           <Loader2 className="animate-spin" size={18} /> Routing...
                        </>
                     ) : (
                        <>
                           <Compass size={18} /> Generate
                        </>
                     )}
                  </button>
               </form>
            ...
            <Card className="mb-4 p-6 bg-surface-container-lowest shadow-[0_12px_32px_rgba(25,28,28,0.06)] border border-primary/10 animate-slide-in">
               ...
               >
                  <div className="flex-1 w-full">
                     ...
                     <div className="relative">
                        ...
                        <input
                           ...
                           placeholder="e.g. Marrakech, Fes, Chefchaouen..."
                           className="w-full pl-12 pr-4 py-3.5 rounded-xl bg-surface-container-low border border-amber-300/40 focus:ring-2 focus:ring-amber-300/30 outline-none transition-all text-on-surface font-semibold text-base"
                           required
                        />
                     </div>
                  </div>
                  ...
                  >
                     {isExploring ? (
                        <>
                           <Loader2 className="animate-spin" size={18} /> Exploring...
                        </>
                     ) : (
                        <>
            ...
            <div className="text-center py-20 bg-surface-container-lowest/50 rounded-[2.5rem] border-2 border-dashed border-outline-variant/30 mt-6 animate-fade-in-up">
               {mode === "roadtrip" ? (
                  <>
                     <Compass
                        size={56}
                        className="mx-auto text-stone-300 mb-6 animate-float"
                     />
                     <h3 className="text-2xl font-bold text-on-surface font-headline mb-3 tracking-tight">
                        Where to next?
                     </h3>
                     <p className="text-on-surface-variant max-w-sm mx-auto text-base leading-relaxed">
                        Enter your starting point and destination to uncover a
                        personalized, scenic route designed by AI.
                     </p>
                  </>
               ) : (
                  <>
                     <Sparkles
                        size={56}
                        className="mx-auto text-amber-300 mb-6 animate-float"
                     />
                     <h3 className="text-2xl font-bold text-on-surface font-headline mb-3 tracking-tight">
                        Pick a city to explore
                     </h3>
                     <p className="text-on-surface-variant max-w-sm mx-auto text-base leading-relaxed">
                        Type the name of any Moroccan city and we'll map out the best
                        places to eat, visit, and experience.
                     </p>
                  </>
               )}
            </div>
         ...
         {error && !isLoading && (
            <div className="text-center py-12 bg-red-50 rounded-[2rem] border border-red-200/50 mt-6 animate-fade-in-up">
               ...
               <h3 className="text-lg font-bold text-red-800 font-headline mb-2">
         ...
         {isLoading && (
            <div className="text-center py-20 bg-surface-container-lowest/80 rounded-[2.5rem] mt-6 flex flex-col items-center justify-center relative overflow-hidden">
               ...
               <Loader2
                  className="animate-spin text-primary mb-6 relative z-10"
                  size={48}
               />
               <h3 className="text-xl font-bold text-on-surface font-headline mb-2 animate-pulse relative z-10">
                  {mode === "roadtrip"
                     ...
                     : `Discovering ${exploreCity}...`}
               </h3>
               <p className="text-on-surface-variant relative z-10">
                  {mode === "roadtrip"
                     ? "Mapping coordinates and analyzing Moroccan topography."
                     : "Finding the best landmarks, food, and hidden gems."}
               </p>
            </div>
         )}
         ...
         {hasResults && !isLoading && (
            <div className="mt-8 animate-fade-in-up">
               <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 px-2">
                  <div>
                     <h2 className="text-2xl font-bold font-headline text-on-surface tracking-tight">
                        {mode === "roadtrip"
                           ? "Your Custom Roadtrip"
                           : `Exploring ${exploreCity}`}
                     </h2>
                     <p className="text-on-surface-variant text-sm font-medium">
                        {mode === "roadtrip"
                           ? `${origin} to ${destination} • ${days} Days • ${itinerary.length} Stops`
                           : `${filteredPois.length} curated points of interest`}
                     </p>
                  </div>
                  ...
                  <button
                        ...
                        const tripData =
                           mode === "roadtrip"
                              ? {
                                 ...
                                 itinerary,
                                 type: "roadtrip",
                                 image: `https://loremflickr.com/800/600/morocco,roadtrip?lock=${Date.now()}`,
                              }
                              : {
                                 title: `Discovering ${exploreCity}`,
                                 city: exploreCity,
                                 itinerary: pois,
                                 type: "explore",
                                 image: `https://loremflickr.com/800/600/morocco,city?lock=${Date.now()}`,
                              };
                        saveTrip(tripData);
                     }}
                     disabled={savedTrips.some(
                        (t) =>
                           t.title ===
                           (mode === "roadtrip"
                     ...
                     className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm transition-all shadow-md ${savedTrips.some(
                        (t) =>
                           t.title ===
                           (mode === "roadtrip"
                              ? `${origin} to ${destination}`
                        ...
                        : "bg-surface-container-high text-on-surface hover:bg-surface-container-highest active:scale-95 border border-outline-variant/30"
                        }`}
                  >
               ...
               >
                  ...
                  {/* Details Area */}
                  <div
                     className="w-full lg:w-2/5 overflow-y-auto pr-2 custom-scrollbar"
                  ...
                  >
                     ...
                     {mode === "explore" && pois && (
                        <div className="flex flex-wrap gap-2 mb-4 animate-fade-in-up">
                           ...
                           {uniqueCategories.map((cat) => {
                              const meta =
                                 CATEGORY_COLORS[cat] || CATEGORY_COLORS.landmark;
                              return (
                                 <button
                                    key={cat}
                                    onClick={() => setCategoryFilter(cat)}
                                    className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-1.5 ${categoryFilter === cat
                                       ? `${meta.bg} ${meta.text} shadow-md scale-105`
                                       : "bg-surface-container-lowest text-on-surface-variant border border-outline-variant/30 hover:scale-105"
                                       }`}
                                 >
                     ...
                     {/* ── CITY EXPLORER DETAILS (Only for Exploration mode) ── */}
                        ...
                        <div className="animate-detail-enter">
                           <Card className="flex flex-col gap-0 p-0 overflow-hidden bg-surface-container-lowest border-0 shadow-[0_12px_32px_rgba(25,28,28,0.08)] rounded-[2rem]">
                              <div className="w-full h-48 relative overflow-hidden shrink-0">
                                 <SmartImage
                                    placeName={activePoi.name}
                                    cityName={exploreCity}
                                    lockId={pois.indexOf(activePoi) + 200}
                                    alt={activePoi.name}
                                 ...
                                 <div className="absolute bottom-4 left-5 text-white">
                                    {(() => {
                                       const meta =
                                          CATEGORY_COLORS[
                                          activePoi.category?.toLowerCase()
                                          ] || CATEGORY_COLORS.landmark;
                                       return (
                                          <span
                                             className={`text-[10px] font-bold uppercase tracking-widest ${meta.bg} ${meta.text} px-3 py-1.5 rounded-full mb-2 inline-flex items-center gap-1 shadow-sm`}
                                          >
                                    ...
                                    <h3 className="text-2xl font-extrabold font-headline tracking-tighter text-white mt-1">
                                       {activePoi.name}
                                    </h3>
                                 </div>
                                 {/* Popularity badge */}
                                 <div className="absolute top-4 right-4 bg-black/50 backdrop-blur-md text-white px-3 py-1.5 rounded-full flex items-center gap-1.5 text-xs font-bold">
                                    <span className="material-symbols-outlined text-amber-400 text-[14px]">
                                       {activePoi.popularity >= 8
                                          ? "local_fire_department"
                                          : activePoi.popularity >= 5
                                             ? "star"
                                             : "diamond"}
                                    </span>
                                 ...
                                 <button
                                    onClick={() =>
                                       isSaved(activePoi.name)
                                          ? removePlace(activePoi.name)
                                          : savePlace({
                                             ...activePoi,
                                             id: activePoi.name,
                                             type: "poi",
                                 ...
                                 >
                                    <span
                                       ...
                                       style={{
                                          fontVariationSettings: isSaved(activePoi.name)
                                             ? "'FILL' 1"
                                             : "'FILL' 0",
                                          color: isSaved(activePoi.name)
                                             ? "#ef4444"
                                             : "white",
                                       }}
                                    >
                                 ...
                                 <div className="mb-4">
                                    <div className="flex items-center justify-between mb-1">
                                       ...
                                       <span className="text-xs font-semibold text-on-surface-variant">
                                          {activePoi.popularity}/10 popularity
                                       </span>
                                    </div>
                                    <div className="w-full h-1.5 bg-stone-100 rounded-full overflow-hidden">
                                       <div
                                          className={`h-full rounded-full transition-all duration-700 ${activePoi.popularity >= 8 ? "bg-gradient-to-r from-amber-400 to-orange-500" : activePoi.popularity >= 5 ? "bg-gradient-to-r from-teal-400 to-primary" : "bg-gradient-to-r from-purple-400 to-indigo-500"}`}
                                          style={{
                                             width: `${(activePoi.popularity || 5) * 10}%`,
                                          }}
                                       ></div>
                                    </div>
                                 </div>
                     ...
                     {!activeMapPoint && (
                        <div
                           className="flex items-center justify-center p-8 bg-surface-container-lowest/50 rounded-[2rem] border-2 border-dashed border-outline-variant/30 animate-fade-in-up"
                           style={{ minHeight: 250 }}
                        >
                           <div className="text-center">
                              <MapPin
                                 size={48}
                                 className="mx-auto text-stone-300 mb-4 animate-float"
                              />
                     ...
                     {/* ── WIKIVOYAGE CARD (Only for Exploration mode) ── */}
                     {mode === "explore" && activeMapPoint && (
                        <Card
                           className="mt-4 p-0 overflow-hidden bg-surface-container-lowest border-0 shadow-[0_8px_24px_rgba(25,28,28,0.06)] rounded-[2rem] animate-fade-in-up"
                           style={{ animationDelay: "0.15s" }}
                        >
                              ...
                              <>
                                 <div className="px-4 pt-3 flex flex-wrap gap-1.5">
                                    {availableSections.map((key) => {
                                       ...
                                       return (
                                          <button
                                             key={key}
                                             onClick={() => setActiveWikiTab(key)}
                                             className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all flex items-center gap-1.5 ${activeWikiTab === key
                                 ...
                                 <div className="p-6 pt-4">
                                    <div className="text-sm text-on-surface leading-relaxed whitespace-pre-line">
                                       {wikiInfo[activeWikiTab] ||
                                          "No information available."}
                                    </div>
                                    <a
                                       href={`https://en.wikivoyage.org/wiki/${encodeURIComponent(exploreCity)}`}
                                       target="_blank"
                     ...
                     <div className="mt-6 flex flex-col gap-3 pb-8">
                        <h4 className="text-sm font-bold text-stone-400 uppercase tracking-widest">
                           {mode === "roadtrip"
                              ? "Journey Overview"
                              : `Places in ${exploreCity}`}
                        </h4>

                        {mode === "roadtrip" &&
                           itinerary?.map((stop, i) => {
                              const isExpanded = activeStop?.stopName === stop.stopName;
                              return (
                                 <div key={i} className="flex flex-col gap-2">
                                    {/* Ticket Header */}
                                    <button
                                       onClick={() => setActiveStop(stop)}
                                       className={`text-left px-5 py-3 rounded-xl border transition-all duration-300 flex items-center gap-4 animate-stagger-in ${isExpanded
                                          ? "bg-primary/5 border-primary text-primary scale-[1.01]"
                                          : "bg-surface-container-lowest border-outline-variant/20 hover:border-primary/30 text-on-surface hover:translate-x-1"
                                          }`}
                                       style={{ animationDelay: `${i * 0.05}s` }}
                                    >
                                       <div
                                          className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs shrink-0 transition-colors ${isExpanded
                                             ? "bg-primary text-white"
                                             : "bg-surface-container text-stone-500"
                                             }`}
                                       >
                                          {i + 1}
                                       </div>
                                    ...
                                    {/* Accordion Content */}
                                    {isExpanded && (
                                       <div className="animate-detail-enter px-1 pb-4">
                                          <Card className="flex flex-col gap-0 p-0 overflow-hidden bg-surface-container-lowest border border-primary/10 shadow-xl rounded-[1.5rem]">
                                             <div className="w-full h-48 relative overflow-hidden shrink-0">
                                                <SmartImage
                                                   placeName={stop.stopName}
                                                   cityName={stop.stopName}
                                                   lockId={i + 100}
                                                   alt={stop.stopName}
                                                ...
                                                <div className="absolute bottom-4 left-5">
                                                   <span className="text-[9px] font-bold uppercase tracking-widest bg-primary/90 text-white px-2 py-1 rounded-md mb-1 inline-block">
                                                      {stop.day || `Stop ${i + 1}`}
                                                   </span>
                                                </div>
                                                <button
                                                   onClick={(e) => {
                                                      e.stopPropagation();
                                                      isSaved(stop.stopName)
                                                         ? removePlace(stop.stopName)
                                                         : savePlace({
                                                            ...stop,
                                                            id: stop.stopName,
                                                            name: stop.stopName,
                                                            category: "Roadtrip Stop",
                                                            city: stop.stopName,
                                                         });
                                                   }}
                                                   className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/40 backdrop-blur-md flex items-center justify-center text-white border border-white/20 hover:scale-110 transition-all z-10"
                                                >
                                                   <span
                                             ...
                                             <div className="p-5">
                                                <p className="text-on-surface-variant leading-relaxed text-xs mb-4 border-l-3 border-primary/20 pl-3">
                                                   {stop.description}
                                                </p>
                                                <div className="space-y-2">
                                                   ...
                                                   <div className="flex flex-wrap gap-2">
                                                      {stop.activities?.map((act, idx) => (
                                                         <span
                                                            key={idx}
                                                            className="text-[11px] bg-surface-container px-3 py-1.5 rounded-lg border border-outline-variant/10 text-on-surface-variant flex items-center gap-1.5"
                                                         >
                                                ...
                                                <button
                                                   onClick={(e) => {
                                                      e.stopPropagation();
                                                      exploreThisCity(stop.stopName);
                                                   }}
                                                   className="w-full mt-5 flex items-center justify-center gap-2 px-4 py-2.5 bg-primary text-white font-bold rounded-xl shadow-md hover:brightness-110 transition-all text-xs"
                                                >
                                                   <Search size={14} />
                                                   See more in {stop.stopName}
                                                </button>
                                             </div>

                                             {/* Nested Wikivoyage */}
                                             {wikiLoading && (
                                                <div className="p-6 text-center border-t border-outline-variant/5">
                                                   <Loader2
                                                      className="animate-spin text-primary mx-auto mb-2"
                                                      size={18}
                                                   />
                                                   <p className="text-[10px] text-on-surface-variant">
                                                      Consulting Wikivoyage...
                                                   </p>
                                                </div>
                                             )}
                                             {!wikiLoading &&
                                                ...
                                                availableSections.length > 0 && (
                                                   <div className="bg-surface-container/30 border-t border-outline-variant/5">
                                                      <div className="px-4 py-3 flex overflow-x-auto no-scrollbar gap-1.5">
                                                         {availableSections.map((key) => {
                                                            const meta = SECTION_META[key] || {
                                                               label: key,
                                                               icon: "article",
                                                            ...
                                                            return (
                                                               <button
                                                                  ...
                                                                  onClick={(e) => {
                                                                     e.stopPropagation();
                                                                     setActiveWikiTab(key);
                                                                  }}
                                                                  className={`px-3 py-1 rounded-full text-[10px] font-bold whitespace-nowrap transition-all flex items-center gap-1.5 ${activeWikiTab === key
                                                                     ? "bg-primary text-white"
                                                      ...
                                                      <div className="px-5 pb-5">
                                                         <div className="text-[11px] text-on-surface-variant leading-relaxed line-clamp-4 overflow-hidden">
                                                            {wikiInfo[activeWikiTab]}
                                                         </div>
                                                         <a
                                                            href={`https://en.wikivoyage.org/wiki/${encodeURIComponent(stop.stopName)}`}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="inline-flex items-center gap-1 mt-3 text-[10px] text-primary font-bold"
                                                         >
                                                            Read Guide{" "}
                                                            <span className="material-symbols-outlined text-[12px]">
                                                               open_in_new
                        ...
                        {mode === "explore" &&
                           filteredPois?.map((poi, i) => {
                              const meta =
                                 CATEGORY_COLORS[poi.category?.toLowerCase()] ||
                                 CATEGORY_COLORS.landmark;
                              ...
                              return (
                                 <button
                                    ...
                                    className={`text-left px-4 py-3 rounded-xl border transition-all duration-300 flex items-center gap-3 animate-stagger-in ${activePoi?.name === poi.name
                                       ? "bg-primary/5 border-primary text-primary scale-[1.01]"
                                       : "bg-surface-container-lowest border-outline-variant/20 hover:border-primary/30 text-on-surface hover:translate-x-1"
                                       }`}
                                    style={{ animationDelay: `${i * 0.04}s` }}
                                 >
                                    <div className="text-xs font-bold text-stone-400 w-5 text-center shrink-0">
                                       #{i + 1}
                                    </div>
                                    <div
                                    ...
                                    >
                                       <span
                                          className={`material-symbols-outlined text-[16px] ${meta.text}`}
                                       >
                                          {meta.icon}
                                       </span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                       <span
                                          className={`material-symbols-outlined text-[12px] ${pop >= 8 ? "text-amber-500" : pop >= 5 ? "text-teal-500" : "text-purple-400"}`}
                                       >
                                          {pop >= 8
                                             ? "local_fire_department"
                                             : pop >= 5
